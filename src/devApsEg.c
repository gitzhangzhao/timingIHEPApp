...
typedef struct MrfEVGStruct
{
  ...
  unsigned int  	VmeEvent  ;	/* and distributed bus data (byte wide) */
  unsigned int  	obsolete_Seq2Addr  ;
  unsigned int  	obsolete_Seq2Data  ;
  unsigned int  	obsolete_Seq1Addr  ;
  unsigned int  	obsolete_Seq1Data  ;

  unsigned int  	Event0Map   ;	/**< Event Mapping Register for external (HW) input 0 */
  unsigned int  	Event1Map   ;	/**< Event Mapping Register for external (HW) input 1 */
  unsigned int  	Event2Map   ;	/**< Event Mapping Register for external (HW) input 2 */
  unsigned int  	Event3Map   ;	/**< Event Mapping Register for external (HW) input 3 */
  unsigned int  	Event4Map   ;	/**< Event Mapping Register for external (HW) input 4 */
  unsigned int  	Event5Map   ;	/**< Event Mapping Register for external (HW) input 5 */
  unsigned int  	Event6Map   ;	/**< Event Mapping Register for external (HW) input 6 */
  unsigned int  	Event7Map   ;	/**< Event Mapping Register for external (HW) input 7 */
  /* Extended registers */
  unsigned int  	MuxCountDbEvEn;   /**< Multiplexed counter distributed bus (byte wide) */
					    /**< enables and event trigger enables (byte wide) */
  unsigned int  	obsolete_Seq1ExtAddr  ;      /**< Extended address SEQ RAM 1 */
  unsigned int  	obsolete_Seq2ExtAddr  ;      /**< Extended address SEQ RAM 2 */
  unsigned int  	Seq1ClockSel  ;     /**< Sequence RAM 1 clock selection */
  unsigned int  	Seq2ClockSel  ;     /**< Sequence RAM 2 clock selection */
  unsigned int  	ACinputControl;     /**< AC related controls register */
  unsigned int  	MuxCountSelect;     /**< reset, Mux seq. trigger enable and Mux select */
  unsigned int  	MuxPrescaler;       /**< Mux prescaler values (25MHz/value) */
  unsigned int  	FPGAVersion ;      /**< FPGA firmware register number (only series 100) */
  UINT32 Reserved_0x30;
  UINT32 Reserved_0x34;
  UINT32 Reserved_0x38;
  UINT32 Reserved_0x3C;
  unsigned int	        RfSelect; 	/**< RF Clock Selection register */
  unsigned int          MxcPolarity;			/**< Mpx counter reset polarity */
  unsigned int          Seq1Addr;    			/**< 11 bit address, 0-2047 (2 kB event RAM) */
  unsigned int          Seq1Data;    			/**< Event code, 8 bits */
  unsigned int          Seq1Time;    			/**< Time offset in event clock from trigger */
  unsigned int          Seq1Pos;     			/**< Current sequence time position */
  unsigned int          Seq2Addr;    			/**< 11 bit address, 0-2047 */
  unsigned int          Seq2Data;    			/**< Event code, 8 bits */
  UINT32 Seq2Time;    			/**< Time offset in event clock from trigger */
  UINT32 Seq2Pos;     			/**< Current sequence time position */
  unsigned int EvanControl; 			/**< Event analyser control register */
  unsigned int EvanEvent;   			/**< Event analyser event code, 8 bits */
  unsigned int EvanTimeH;   			/**< Bits 63-32 of event analyser time counter */
  UINT32 EvanTimeL;   			/**< Bits 31-0 of event analyser time counter */
  unsigned int    uSecDivider;               /* Reserved_0x68; */
  unsigned int DataBufSize;   /*2006 May 16 leige*/
  UINT32 Reserved_0x6C;
  UINT32 Reserved_0x70;
  UINT32 Reserved_0x74;
  UINT32 Reserved_0x78;
  UINT32 Reserved_0x7C;
  unsigned int    FracDivControl;  	/**< Micrel SY87739L programming word */
  UINT32 DelayRf;		
  UINT32 DelayRx;		
  UINT32 DelayTx;
  UINT32 AdiControl;		
...
STATIC long EgSetACinputDivisor(EgLinkStruct *pParm, unsigned int Divisor, unsigned int DlySel);
...
int EgConfigureA24(int Card, unsigned long CardAddress, unsigned short divider)
{
  unsigned long Junk;
    ...
    if (ConfigureLock != 0)
  {
    printf("EgConfigureA24, ConfigureLock != 0\n");
    /*epicsPrintf("devApsEg: Cannot change configuration after init.  Request ignored\n");*/
    return(-1);
  }
  if (Card > 21 || Card < 1) 
  ...
  if (CardAddress > 0xffffffff)
  {
    printf("devApsEg: Card address invalid, must be 0x0000-0xffff\n");
    /*epicsPrintf("devApsEg: Card address invalid, must be 0x0000-0xffff\n");*/
    return(-1);
  }
 ...
 if(sysBusToLocalAdrs(VME_AM_STD_USR_DATA, (char*)CardAddress, (char**)&EgLink[Card].pEg) !=OK)
  {
    printf("devApsEg: Failure mapping requested A24 address\n");
    /*epicsPrintf("devApsEg: Failure mapping requested A16 address\n");*/
    EgLink[Card].pEg = NULL;
    return(-1);
  }
	...
	{
	    printf("VME address is %x\n", EgLink[Card].pEg);
	}
  
  
  printf("address is %x\n", EgLink[Card].pEg);
  printf("VME address is %x\n", &EgLink[Card].pEg->Control);
  printf("EventMask is %x\n", &EgLink[Card].pEg->EventMask);
  printf("VmeEvent is %x\n", &EgLink[Card].pEg->VmeEvent);
  printf("obsolete_Seq2Addr is %x\n", &EgLink[Card].pEg->obsolete_Seq2Addr);
  printf("obsolete_Seq1Data is %x\n", &EgLink[Card].pEg->obsolete_Seq1Data);

  if (vxMemProbe((char*)&(EgLink[Card].pEg->Control), READ, sizeof(int), (char*)&Junk) != OK)
  {
    printf("devApsEg vxMemProbe: Failure probing for event generator... Card disabled\n");
    /*epicsPrintf("devApsEg: Failure probing for event generator... Card disabled\n");*/
    EgLink[Card].pEg = NULL;
    return(-1);
  }
	...
	{
	    printf("Memory probe value is %x\n", Junk);
	}
  ...
  switch (divider) {
     ...
     case 5:
        ...
        break;
     case 10:
        EgLink[Card].pEg->RfSelect = 0x01c9;
        EgLink[Card].pEg->FracDivControl = 0x025b43ad;
        EgLink[Card].pEg->uSecDivider = 50;
        break;
     default: 
...
printf("EgLoadRamList %d\n", (int)Ram);  
  ...
  while (pNode != NULL)
  {
    ...
    if (AltFlag!=1 && pEgevent->ram !=Ram) { printf("EgLoadRamList do nothing\n"); } else
    {
...
printf("here2!\n");
    ...
    for (j=0;j<EgNumLinks;j++)
    {
      ...
      semTake (EgLink[j].EgLock, WAIT_FOREVER);

      if (EgGetMode(&EgLink[j], 1) != egMOD1_Alternate)
      { /* Not in ALT mode, each ram is autonomous */
     
  printf("Not in ALT mode, each ram is autonomous\n"); 
      ...
      {
       printf("We are in ALT mode, we have one logical RAM\n");
       
	/* See if RAM 1 is available for downloading */
	if ((EgLink[j].Ram1Dirty != 0)||(EgLink[j].Ram2Dirty != 0))
	{
          printf("See if RAM is available for downloading\n");
	  if (EgGetAltStatus(&EgLink[j], 1) == 0)
	  {
            printf("RAM 1 is available for downloading\n");
	    EgClearSeq(&EgLink[j], 1);
	    EgLoadRamList(&EgLink[j], 1);
            printf("RAM 1 is downloaded\n");
	    EgLink[j].Ram1Dirty = 0;
	    EgLink[j].Ram2Dirty = 0;
	  ...
	  {
            printf("RAM 2 is available for downloading\n");
	    EgClearSeq(&EgLink[j], 2);
	    EgLoadRamList(&EgLink[j], 2);
	    EgLink[j].Ram1Dirty = 0;
	    EgLink[j].Ram2Dirty = 0;
      ...
      /* Unlock the EG link */
      i=semGive (EgLink[j].EgLock);		/******** UNLOCK *************/
      printf("EgRamTask semGive %d\n", i);
      printf("Spinning is %d\n", Spinning);
      }
    }
  }
...
STATIC long EgInitDev(int pass)
{
  ...
  { /* Do any hardware initialization (no records init'd yet) */
        ...
        if (EgCheckCard(j) == 0)
	{
          ...
          printf("EgLock sem is created %d \n",EgLink[j].EgLock);
...
printf("now return from EgInitDev\n");
  return(0);
}
...
{
int i;
  printf("EgScheduleRamProgram begin \n");
 i=semGive(EgRamTaskEventSem);
  printf("segGive %d\, ", i);
  return(0);
}
/**
...
STATIC long EgInitEgRec(struct egRecord *pRec)
{
  ...
  printf("EgLink[pRec->out.value.vmeio.card].RFselect %x\n", EgLink[pRec->out.value.vmeio.card].RFselect);
  printf("RFSelect - %x\n", pEg->RfSelect);    
  } 
...
printf("EgProcEgRec begin\n");
  ...
  printf("pLink->EgLock %d",pLink->EgLock);
  printf("It's here1!\n"); 
...
adiv = EgGetACinputDivisor(pLink, 0);
printf("ADIV is %d," , adiv);
 
  if ( pRec->adiv != EgGetACinputDivisor(pLink, 0))
	EgSetACinputDivisor(pLink, pRec->adiv, 0);

printf("pRec->adiv is %x\n", pRec->adiv);
...
*/
  ...
  i=semGive (pLink->EgLock);
  printf("EgProEgRec semGive %d \n",i); 
...
double RamSpeed;
  ...
  i=semGive(EgLink[pRec->out.value.vmeio.card].EgLock);
  printf("EgInitEgEventRec semGive %d \n", i); 
  return(0);
}
/**
 *
...
STATIC long EgProcEgEventRec(struct egeventRecord *pRec)
{
  ...
  if (pRec->ram != pRec->lram)
  {
    ...
    EgPlaceRamEvent(pRec);
    i=semGive(EgLink[pRec->out.value.vmeio.card].EgLock);
    printf("EgProEgEventRec semGive1 %d \n", i); 
    pRec->lram = pRec->ram;
  }
  ...
  if (pRec->dely != pRec->ldly)
  {
    ...
    EgPlaceRamEvent(pRec);
    i=semGive(EgLink[pRec->out.value.vmeio.card].EgLock);
    printf("EgProEgEventRec semGive2 %d \n",i); 
...
struct {
  ...
  long (*p4)();
  long (*p5)();
} devEgEvent={ 5, NULL, NULL, EgInitEgEventRec, NULL, EgProcEgEventRec};

/******************************************************************************
 *
...
STATIC long EgSetACinputDivisor(EgLinkStruct *pParm, unsigned int Divisor, unsigned int DlySel)
{
  int div=0;
  volatile MrfEVGStruct *pEg = pParm->pEg;
  unsigned int LocalDivisor=0;
  DlySel <<= 8; 
  DlySel &= 0x00000100; 

  LocalDivisor = Divisor & 0x000000ff;

printf("EgSetACinputDevisor begin, %d\n", DlySel);
  ...
  if(DlySel) 
	pEg -> ACinputControl |= DlySel;
  	...
  	pEg -> ACinputControl &= ~(0x00000100);
	
     pEg -> ACinputControl = (pEg -> ACinputControl & 0x0000ff00) | LocalDivisor;
  
  div = pEg -> ACinputControl ;
  printf("LocalDivisor is %x \n",LocalDivisor);
  printf("pEg -> ACinputControl is %x\n", &pEg -> ACinputControl);
  printf("pEg -> ACinputControl is %x\n", pEg -> ACinputControl);
  printf("pEg -> ACinputControl is %x\n", div);
printf("EgSetACinputDevisor end\n");
...
STATIC unsigned long EgGetMuxPrescaler(EgLinkStruct *pParm, unsigned short Channel)
{
  ...
  printf("EgGetMuxPrescaler walue is %x\n", PreScaleVal); 
...
#endif
  printf("EgEnableAltRam\n");
  printf("pEg->Control %X\n", pEg->Control);
