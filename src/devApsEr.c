...
typedef struct MrfErStruct
{
  ...
  unsigned int		EventRamAddr;
  unsigned int		EventRamData;
  unsigned int		OutputPulseEnables;
  unsigned int		OutputLevelEnables;
  unsigned int		TriggerEventEnables;
  unsigned int		EventCounterLo;
  unsigned int		EventCounterHi;
  unsigned int		TimeStampLo;
  unsigned int		TimeStampHi;

  unsigned int		EventFifo; /* LSB = event #, MSB = LSB of time stamp counter */
  unsigned int		EventTimeHi; /* bits 23-8 of the time stamp counter */
  unsigned int		DelayPulseEnables;
  unsigned int		DelayPulseSelect; 
  /* OTP pulses have delays and widths. See HW technical reference */
  unsigned int		PulseDelay;
  unsigned int		PulseWidth;
  unsigned int		IrqVector;
  unsigned int		IrqEnables;
  /* Extended registers */
  unsigned int		DBusEnables; /* Distributed bus enable register */
  unsigned int		DBusData; /* Distributed bus data register (read only) */
  unsigned int		DelayPrescaler; /* Programmable delay pulse clock prescaler */
  unsigned int		EventCounterClock; /* Event counter clock prescaler */
  unsigned int		Resvd1; /* 0x2c */
  unsigned int		FPGAVersion; /* FPGA Firmware version number */
  unsigned int		Resvd3; /* 0x30 */
  unsigned int		Resvd4; /* 0x32 */
  unsigned int		Resvd5; /* 0x34 */
  unsigned int		Resvd6; /* 0x36 */
  unsigned int		Resvd7; /* 0x38 */
  unsigned int		Resvd8; /* 0x3a */
  unsigned int		Resvd9; /* 0x3c */
  unsigned int		Resvd10; /* 0x3e */
  unsigned int		FP0Map; /* 0x040, Front panel output mapping register */
  unsigned int		FP1Map; /* 0x042, Front panel output mapping register */
  unsigned int		FP2Map; /* 0x044, Front panel output mapping register */
  unsigned int		FP3Map; /* 0x046, Front panel output mapping register */
  unsigned int		FP4Map; /* 0x048, Front panel output mapping register */
  unsigned int		FP5Map; /* 0x04a, Front panel output mapping register */
  unsigned int		FP6Map; /* 0x04c, Front panel output mapping register */
  unsigned int		uSecDivider; /*Resvd11; 0x4e*/
  unsigned int		ExtEventCode;  /* 0x50 */
  ...
  unsigned int		Presc0;   /* 0x074, Front panel clock #0 prescaler */
  unsigned int		Presc1;	  /* 0x076, Front panel clock #1 prescaler */
  unsigned int		Presc2;   /* 0x078, Front panel clock #2 prescaler */
  /*unsigned int	 	Resvd13;*/ /* 0x7a*/
  unsigned int		DataBufCtrl; /* 0x7a*/ /*leige */
...
int			ErDebug = 20;  /*leige move it to erDefs.h*/
int			ErStallIrqs = 1;
...
int ErConfigureA24(int Card, unsigned long CardAddress, unsigned int IrqVector, 
		unsigned int IrqLevel)
{
  unsigned long 	Junk;
  ...
  if(sysBusToLocalAdrs(VME_AM_STD_USR_DATA, (char*)CardAddress,
		       (char**)&ErLink[Card].pEr)!=OK)
  {
    epicsPrintf("devApsEr: Failure mapping requested A16 address\n");
    ErLink[Card].pEr = NULL;
